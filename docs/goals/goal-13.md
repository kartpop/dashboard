# Goal 13 — Frontend shell: settings modal, per-user feature flags, news settings
...
- **2. Allowed emails → DB-backed users with feature flags (owner-only panel).**
  ...
  - **Feature flags already DB-backed (goal 11)**: `allowed_email.features` + the News
    ...
    Goal 13 only enriches the management UI (chip input, more features) over that column —
...
- Deploy/owner docs: allowlist/feature-flag management note (`goal-13-owner-steps.md` if
